...
run_CV_cyclic_helper_y0 <- function(M, M_fixed, K_fold_repetitions, K, X, Z, times, delta, boost_intercepts_continually=boost_intercepts_continually) {
  # M varies; corresponds to genes and thus y0
  # M_fixed does not vary; it corresponds to clinical and thus mu
  ...
  for (repeated_K_fold_iteration in 1:K_fold_repetitions) {
    ...
    for (k in 1:K) {
      ...
      result <- cyclic_boosting_run(
        times=times_without_k,
        delta=delta_without_k,
        X=X_without_k,
        Z=Z_without_k,
        m_stop_y0=M,
        m_stop_mu=M_fixed,
